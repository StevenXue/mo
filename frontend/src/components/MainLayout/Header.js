...
function Header({location, login, history, dispatch, allRequest,message}) {
  ...
  const switchMessage = (e) => {
    switch(e.message_type) {
      ...
      case 'deploy':
        return <p>{e.user_ID} 上线了您关注的{translatorTemp[e.project_type]}  {e.project_name}</p>
      case 'publish':
        return <p>{e.user_ID} 发布了您关注的{translatorTemp[e.project_type]}  {e.project_name}</p>
      case 'deploy_request':
        return <p>{e.user_ID} 为您的答案{e.user_request_title} 上线了{translatorTemp[e.project_type]}  {e.project_name}</p>
      case 'publish_request':
        return <p>{e.user_ID} 为您的答案{e.user_request_title} 发布了{translatorTemp[e.project_type]}  {e.project_name}</p>
    }
  }
