...
/**
 * Initialization data for the deploy-extension extension.
 */
const extension: JupyterLabPlugin<void> = {
  id: '@jupyterlab/deploy-extension:plugin',
  autoStart: true,
  requires: [IFileBrowserFactory],
  activate: (app: JupyterLab, fb: IFileBrowserFactory) => {
    // add to filebrowser toolbar
    fb.defaultBrowser.toolbar.addItem('deploy', createDeployButton());
    ...
    return new ToolbarButton({
      ...
      onClick: () => {
        return showDialog({
          ...
          body: new DeployForm(projectType, () => {
            console.log('click');
          }),
          ...
          if (projectType === 'app' && !result.value.selectFile) {
            ...
            if (result.value.versionNumber) {
              publish({
                ...
                onJson: () => {
                  message.success(`${projectType} publish success!`);
                },
              });
            } else {
              deploy({
                ...
                commitMsg: result.value.commitMsg,
                onJson: () => {
                  message.success(`${projectType} deploy success!`);
                  // hide();
                },
              });
            }
            window.location.replace(`/#/workspace/${projectId}?type=${projectType}`);
