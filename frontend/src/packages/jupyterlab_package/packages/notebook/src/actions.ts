...
export namespace NotebookActions {
     ...
     */
    export function splitCell(widget: Notebook): void {
     ...
     */
    export function mergeCells(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * This action can be undone.
     */
    export function deleteCells(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     */
    export function insertAbove(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     */
    export function insertBelow(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * The new cell will be the active cell.
     */
    export function insertCode(widget: Notebook, code: nbformat.MultilineString): void {
        ...
        let cell = model.contentFactory.createCodeCell({cell: codeCell});
     ...
     * The new cell will be the active cell.
     */
    export function insertInitCode(widget: Notebook, code: nbformat.MultilineString, session?: IClientSession): void {
     ...
     * The new cell will be the active cell.
     */
    export function insertCodeBelow(widget: Notebook, code: nbformat.MultilineString): void {
     ...
     * @param widget = The target notebook widget.
     */
    export function moveDown(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * @param widget - The target notebook widget.
     */
    export function moveUp(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     */
    export function changeCellType(widget: Notebook, value: nbformat.CellType): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     */
    export function run(widget: Notebook, session?: IClientSession): Promise<boolean> {
        if (!widget.model || !widget.activeCell) {
     ...
     */
    export function runAndAdvance(widget: Notebook, session?: IClientSession): Promise<boolean> {
        if (!widget.model || !widget.activeCell) {
            return Promise.resolve(false);
        }
        let state = Private.getState(widget);
     ...
     */
    export function runAndInsert(widget: Notebook, session?: IClientSession): Promise<boolean> {
        if (!widget.model || !widget.activeCell) {
            return Promise.resolve(false);
        }
        let state = Private.getState(widget);
     ...
     */
    export function runAll(widget: Notebook, session?: IClientSession): Promise<boolean> {
        if (!widget.model || !widget.activeCell) {
            return Promise.resolve(false);
        }
        let state = Private.getState(widget);
     ...
     */
    export function runAllAbove(widget: Notebook, session?: IClientSession): Promise<boolean> {
     ...
     * The last cell in the notebook will be activated and scrolled into view.
     */
    export function runAllBelow(widget: Notebook, session?: IClientSession): Promise<boolean> {
        if (!widget.model || !widget.activeCell) {
            return Promise.resolve(false);
        }
        let state = Private.getState(widget);
     ...
     * The existing selection will be cleared.
     */
    export function selectAbove(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
     ...
     * The existing selection will be cleared.
     */
    export function selectBelow(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        if (widget.activeCellIndex === widget.widgets.length - 1) {
     ...
     */
    export function extendSelectionAbove(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        // Do not wrap around.
     ...
     * The new cell will be activated.
     */
    export function extendSelectionBelow(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        // Do not wrap around.
     ...
     */
    export function selectAll(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        each(widget.widgets, child => {
     ...
     * @param widget - the targe notebook widget.
     */
    export function deselectAll(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        widget.deselectAll();
     ...
     * @param widget - The target notebook widget.
     */
    export function copy(widget: Notebook): void {
     ...
     */
    export function cut(widget: Notebook): void {
     ...
     * This action can be undone.
     */
    export function paste(widget: Notebook, mode: 'below' | 'above' | 'replace' = 'below'): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
     ...
     */
    export function undo(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     */
    export function redo(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     */
    export function toggleAllLineNumbers(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * The widget `mode` will be preserved.
     */
    export function clearOutputs(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * The widget `mode` will be preserved.
     */
    export function clearAllOutputs(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * @param widget - The target notebook widget.
     */
    export function hideCode(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * @param widget - The target notebook widget.
     */
    export function showCode(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * @param widget - The target notebook widget.
     */
    export function hideAllCode(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * @param widget - The target notebook widget.
     */
    export function showAllCode(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * @param widget - The target notebook widget.
     */
    export function hideOutput(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * @param widget - The target notebook widget.
     */
    export function showOutput(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * @param widget - The target notebook widget.
     */
    export function hideAllOutputs(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * @param widget - The target notebook widget.
     */
    export function showAllOutputs(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * @param widget - The target notebook widget.
     */
    export function enableOutputScrolling(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     * @param widget - The target notebook widget.
     */
    export function disableOutputScrolling(widget: Notebook): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     */
    export function setMarkdownHeader(widget: Notebook, level: number) {
        if (!widget.model || !widget.activeCell) {
            return;
        }
        let state = Private.getState(widget);
     ...
     */
    export function trust(widget: Notebook): Promise<void> {
        if (!widget.model) {
...
namespace Private {
     ...
     */
    export function getState(widget: Notebook): IState {
        return {
     ...
     */
    export function handleState(widget: Notebook, state: IState, scrollIfNeeded = false): void {
     ...
     */
    export function handleRunState(widget: Notebook, state: IState, scroll = false): void {
        if (state.wasFocused || widget.mode === 'edit') {
            widget.activate();
        }
        if (scroll) {
     ...
     * Clone a cell model.
     */
    export function cloneCell(model: INotebookModel, cell: ICellModel): ICellModel {
        switch (cell.type) {
            case 'code':
     ...
     */
    export function runSelected(widget: Notebook, session?: IClientSession): Promise<boolean> {
        widget.mode = 'command';
        let selected: Cell[] = [];
     ...
     */
    export function copyOrCut(widget: Notebook, cut: boolean): void {
        if (!widget.model || !widget.activeCell) {
            return;
        }
