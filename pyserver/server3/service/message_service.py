...
from server3.business import UserBusiness
...
def create_message(sender, message_type, receivers, user=None, **kwargs):
    if user and isinstance(user, (str, ObjectId)):
        user = UserBusiness.get_by_user_object_id(user)
    if isinstance(sender, (str, ObjectId)):
        sender = UserBusiness.get_by_user_object_id(sender)
    receivers = [UserBusiness.get_by_user_object_id(r)
