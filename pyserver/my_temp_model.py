...
def model_function(x_train, y_train, x_test, y_test):
    with graph.as_default():
        model = Sequential()
        model.add(Dense(units={{choice([16, 32, 64])}}, activation={{choice(['linear', 'relu'])}}, input_shape=[4]))
        model.add(Dropout(rate={{uniform(0, 0.5)}}))
        model.add(Dense(units=2, activation='softmax'))
        model.compile(optimizer=SGD(lr={{uniform(0, 0.05)}}), loss={{choice(['categorical_crossentropy', 'hinge'])}}, metrics=['acc'])
